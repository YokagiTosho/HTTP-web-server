...
static int response_to_raw(const response_t *response, struct raw_response *raw)
{
	...
	response_add_str(http_raw, &offset, "%s %d %s\r\n",
			http_version_to_str(response->http_version),
			response->status_code,
			response->verbose);

	if (response->content_length > 0) {
		response_add_str(http_raw, &offset, "Content-Length: %d\r\n", response->content_length);
	}
	if (response->connection) {
		response_add_str(http_raw, &offset, "Connection: %s\r\n", response->connection);
	}
	if (response->content_type) {
		response_add_str(http_raw, &offset, "Content-Type: %s\r\n", response->content_type);
	}
	if (response->content_language) {
		response_add_str(http_raw, &offset, "Content-Language: %s\r\n", response->content_language);
	}
	if (response->location) {
		response_add_str(http_raw, &offset, "Location: %s\r\n", response->location);
	}
	if (response->set_cookie) {
		response_add_str(http_raw, &offset, "Set-Cookie: %s\r\n", response->set_cookie);
	}
	if (response->content_encoding) {
		response_add_str(http_raw, &offset, "Content-Encoding: %s\r\n", response->content_encoding);
	}
	if (response->transfer_encoding) {
		response_add_str(http_raw, &offset, "Transfer-Encoding: %s\r\n", response->transfer_encoding);
	}
	if (response->date) {
		response_add_str(http_raw, &offset, "Date: %s\r\n", response->date);
	}
	if (response->last_modified) {
		response_add_str(http_raw, &offset, "Last-Modified: %s\r\n", response->last_modified);
	}
	if (response->server) {
		response_add_str(http_raw, &offset, "Server: %s\r\n", response->server);
	}

	response_add_str(http_raw, &offset, "\r\n"); // end of headers
...
int send_response(int fd, const response_t *response)
{
...
#if 0
#ifdef DEBUG
	printf("After \"response_to_raw()\": %s\n", raw.bytes);
#endif
#endif
